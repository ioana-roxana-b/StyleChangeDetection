...
def pipeline_wan():
    ...
    preprocessed_text = preprocessing.preprocessing(sentences, punctuations=True, stopwords=False, lemmatizer=True)

    # with open("preprocessed_acts.json", "w") as file:
    #     json.dump(preprocessed_text, file, indent=4)

    wans = preprocessing.construct_wans(preprocessed_text, include_pos=True, output_dir="dos_wans")

    #wans = preprocessing.load_wans("wans.json")

    features = graph_features.extract_features(wans)

    graph_features.save_features_to_json(features, filename="features_en.json")

    graph_features.extract_lexical_syntactic_features(features, top_n=10, filename = "graph_features_en.csv")
